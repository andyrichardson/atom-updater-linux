...
export class Installer {
  ...
  installUpdate(){
    let install = (this.pkgType == "rpm") ? this.installRpm : this.installDeb;

    install((code) => {
      if(code == 0){
        atom.notifications.addSuccess("Atom has been successfully updated.");
      }
      else{
        atom.notifications.addError("Something went wrong while trying to install atom");
      }
    })
  }
