...
export class Updater {
  ...
  needUpdate(avaliable, current) {
    console.log([
      [avaliable.major, avaliable.minor, avaliable.build].join('.'),
      [current.major, current.minor, current.build].join('.')
    ]);

    if(!atom.config.get('updater-notify.acceptTestVersion') && avaliable.stage)
      return false;
