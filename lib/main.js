...
let main = {
  ...
  automaticChecker() {
    ...
    if(atom.config.get('atom-updater-linux.pkgType') === 'Debian, Ubuntu')
      ...
      pkgType = '.rpm';
    else if(atom.config.get('atom-updater-linux.pkgType') === 'Mac OSX')
      pkgType = '.nupkg';
              ...
              '<div>',
                ...
                info.assets.map((asset) => {
                  if(asset.name.indexOf(pkgType) >= 0)
                    return '<div class="btn btn-primary btn-install-update" data-target="' + asset.browser_download_url + '">Download and install</div>';
                  else
                    return '';
                }).join(''),
                '</ul>',
        ...
        if(info) {
          atom.notifications.addInfo(
            [
              ...
              '<p>Latest Version: <a href="' + info.html_url + '">' + info.name + '</a></p>',
              '<div class="btn btn-primary btn-install-update" data-target="' + "testing only" + '">Install</div>' //remove
            ].join('')
          );
        }
        else
